...
def get_daily_question(user_id: int):
    ...
    question = next(
        (q for q in questions_library if q["id"] == current_question_id), None
    )
    if not question:
        print(
...
def submit_quiz_answer(payload: AnswerPayload):
    ...
    if question["answer"] == payload.submitted_answer:
        ...
        last_played = user["quiz"]["last_played"]
        if last_played is None or last_played.date() != datetime.now().date():
